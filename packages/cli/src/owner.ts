...
// The row as stored, refresh token still encrypted. Reading the token back is
// requireLocalRefreshToken's job, and the only place it is in the clear.
...
export async function saveLocalQueueOwner(db: Db, cipher: Cipher, refreshToken: string): Promise<void> {
  await saveQueueOwner(db, {
    spotifyUserId: LOCAL_QUEUE_OWNER,
    displayName: 'Standalone CLI',
    refreshToken: await cipher.encrypt(refreshToken),
...
export async function requireLocalRefreshToken(db: Db, cipher: Cipher): Promise<string> {
  const owner = await localQueueOwner(db);
  if (!owner) throw new Error('No Spotify authorization yet. Run `earshot auth` first.');
  return cipher.decrypt(owner.refreshToken);
}

// Spotify rotates refresh tokens on its own schedule.
export async function saveLocalRefreshToken(db: Db, cipher: Cipher, refreshToken: string): Promise<void> {
  await saveRefreshToken(db, LOCAL_QUEUE_OWNER, await cipher.encrypt(refreshToken));
}
